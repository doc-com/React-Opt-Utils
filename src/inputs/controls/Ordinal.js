...
const validateOrdinal = (value, occurrences, translate) => {
    let error = "";
...
const Ordinal = (props) => (
    <FastField name={props.path}
               validate={(value) => {
                   return validateOrdinal(value, props.control.occurrences, props.translate)
               }}
               render={
                   ({field, form}) => {
                           ...
                           <Form.Group as={Col} controlId={props.path}>
                               <InputGroup>
                                   <Form.Control as="select"
                                                 ...
                                                 onChange={(e) => {
                                                     if (e.target.value) {
                                                         let selectedEntry = props.control.ordinalList.filter((entry) => {
                                                         ...
                                                         if (selectedEntry && selectedEntry[0]) {
                                                             form.setFieldValue(props.path, {
                                                                 ...
                                                                 ordinalValue: selectedEntry[0].ordinalValue,
                                                                 terminologyId: selectedEntry[0].terminologyId,
                                       ...
                                       {[<option hidden disabled value={""}
                                                 key={`${props.path}-opt-1`}>{props.translate('-- Select an option --')}</option>, ...props.control.ordinalList.map(
